...
export class ModalInstance {
    ...
    public suffix: string = '.ng2-bs4-modal';
    public shownEventName: string = 'shown.bs.modal' + this.suffix;
    public hiddenEventName: string = 'hidden.bs.modal' + this.suffix;
    public $modal: any;
    ...
    public hide(): Promise<ModalResult> {
    ...
    public init() {
        ...
        this.hidden = Observable.fromEvent(this.$modal, this.hiddenEventName)
            .map(() => {
                let result = (!this.result || this.result === 0)
                    ? 2 : this.result;
